                    ...
                    newsCtg.articles.slice(0, 8).map(({ title, url, urlToImage, author, source }) => (
                        <Link to={url} key={title} className="flex mt-10 flex-col items-center">
                            <div className="flex justify-between w-96">
                                <div className="flex">
                                    <div className={`w-6 h-6 rounded-full ${darkTheme ? "bg-[#59777e]" : "bg-[#dd8787]"}`}></div>
                                    <p className={`${darkTheme ? "text-cream" : "text-black"}`}>{author}</p>
                                </div>
                                <div>
                            ...
                            <div className="flex justify-center text-center w-[70%]">
                                <p className={` ${darkTheme ? "text-cream" : "text-black"} font-bold`}>{title}</p>
                            </div>
                        </Link>
                    ))
                )}
